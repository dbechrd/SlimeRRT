...
#if 1
    Vector2 randXY = { 0 };
    randXY.x = dlb_rand_variance(1.0f);
    randXY.y = dlb_rand_variance(1.0f);
    randXY = v2_scale(v2_normalize(randXY), METERS_TO_PIXELS(1.0f) * dlb_rand_float(0.0f, 1.2f));
    float randZ = dlb_rand_float(0.0f, METERS_TO_PIXELS(0.3f));
    particle->body.velocity = (Vector3){ randXY.x, randXY.y, randZ };
