...
{
    DLB_ASSERT(buffer);
    DLB_ASSERT(bufferSize);
    //DLB_ASSERT(bufferSize % 4 == 0);  // Must be aligned to word boundary
};
...
void BitStream::Align()
{
    ...
    if (remainderBits) {
        // NOTE: If DLB_ASSERT fails in Reader mode, alignment padding corruption was detected; expected zero padding
        uint32_t value = 0;
        Process(value, 8 - remainderBits, 0, 0);
    }
    DLB_ASSERT(scratchBits % 8 == 0);
    DLB_ASSERT(bitsProcessed % 8 == 0);
}
...
void BitStream::ProcessInternal(uint32_t &word, uint8_t bits)
{
    ...
    switch (mode) {
        case Mode::Reader: {
            ...
            DLB_ASSERT(bitsProcessed + bits <= bufferBits);
            ...
            DLB_ASSERT((uint64_t)word < ((uint64_t)1 << bits));  // Ensure there's enough bits to fit the whole value
...
{
    DLB_ASSERT(bits <= sizeof(value) * 8);
    if (mode == Mode::Writer) {
        DLB_ASSERT(value >= min);
        DLB_ASSERT(value <= max);
    }
...
void BitStream::Process(uint64_t &value, uint8_t bits, uint64_t min, uint64_t max)
{
    //DLB_ASSERT(bits <= sizeof(value) * 8);
    DLB_ASSERT(bits == 64);
    if (mode == Mode::Writer) {
    ...
    DLB_ASSERT(sizeof(uint32_t) * 2 == sizeof(uint64_t));
...
void BitStream::Process(double &value, uint8_t bits, double min, double max)
{
    ...
    DLB_ASSERT(sizeof(uint32_t) * 2 == sizeof(double));
    uint32_t word0 = ((uint32_t *)&value)[0];
    uint32_t word1 = ((uint32_t *)&value)[1];
    ProcessInternal(word0, 32);
    ProcessInternal(word1, 32);
...
{
    if (mode == Mode::Writer) {
        DLB_ASSERT(value >= STRING_ASCII_MIN);
        DLB_ASSERT(value <= STRING_ASCII_MAX);
    }
...
void BitStream::Flush()
{
    if (mode == Mode::Writer && scratchBits) {
        DLB_ASSERT((wordIndex + 1) * 32 < bufferBits);
