...
void PlayerControllerState::Query(bool processMouse, bool processKeyboard, bool freeCamera)
{
    ...
    // Normal gameplay events
        ...
        if (processKeyboard) {
            ...
            if (IsKeyPressed(KEY_ONE)  ) { selectSlot = SlotId_Hotbar_0; }
            if (IsKeyPressed(KEY_TWO)  ) { selectSlot = SlotId_Hotbar_1; }
            if (IsKeyPressed(KEY_THREE)) { selectSlot = SlotId_Hotbar_2; }
            if (IsKeyPressed(KEY_FOUR) ) { selectSlot = SlotId_Hotbar_3; }
            if (IsKeyPressed(KEY_FIVE) ) { selectSlot = SlotId_Hotbar_4; }
            if (IsKeyPressed(KEY_SIX)  ) { selectSlot = SlotId_Hotbar_5; }
            if (IsKeyPressed(KEY_SEVEN)) { selectSlot = SlotId_Hotbar_6; }
            if (IsKeyPressed(KEY_EIGHT)) { selectSlot = SlotId_Hotbar_7; }
            if (IsKeyPressed(KEY_NINE))  { selectSlot = SlotId_Hotbar_8; }
            if (IsKeyPressed(KEY_ZERO))  { selectSlot = SlotId_Hotbar_9; }
