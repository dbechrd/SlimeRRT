...
PlayerControllerState PlayerControllerState::Query(bool processMouse, bool processKeyboard, bool freeCamera)
{
    ...
    // Normal gameplay events
        ...
        if (processKeyboard) {
            ...
            if (IsKeyPressed(KEY_ONE)  ) { input.selectSlot = (uint32_t)PlayerInventorySlot::Hotbar_1; }
            if (IsKeyPressed(KEY_TWO)  ) { input.selectSlot = (uint32_t)PlayerInventorySlot::Hotbar_2; }
            if (IsKeyPressed(KEY_THREE)) { input.selectSlot = (uint32_t)PlayerInventorySlot::Hotbar_3; }
            if (IsKeyPressed(KEY_FOUR) ) { input.selectSlot = (uint32_t)PlayerInventorySlot::Hotbar_4; }
            if (IsKeyPressed(KEY_FIVE) ) { input.selectSlot = (uint32_t)PlayerInventorySlot::Hotbar_5; }
