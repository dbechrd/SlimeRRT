...
void PlayerControllerState::Query(bool processMouse, bool processKeyboard, bool freeCamera)
{
    ...
    // Normal gameplay events
        ...
        if (processKeyboard) {
            ...
            if (IsKeyPressed(KEY_ONE)  ) { selectSlot = PlayerInvSlot_Hotbar_0; }
            if (IsKeyPressed(KEY_TWO)  ) { selectSlot = PlayerInvSlot_Hotbar_1; }
            if (IsKeyPressed(KEY_THREE)) { selectSlot = PlayerInvSlot_Hotbar_2; }
            if (IsKeyPressed(KEY_FOUR) ) { selectSlot = PlayerInvSlot_Hotbar_3; }
            if (IsKeyPressed(KEY_FIVE) ) { selectSlot = PlayerInvSlot_Hotbar_4; }
            if (IsKeyPressed(KEY_SIX)  ) { selectSlot = PlayerInvSlot_Hotbar_5; }
            if (IsKeyPressed(KEY_SEVEN)) { selectSlot = PlayerInvSlot_Hotbar_6; }
