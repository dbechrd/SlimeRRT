...
u32 dlb_rand();
int dlb_rand_int(int min, int max);
...
// min and max are both inclusive bounds
int dlb_rand_int(int min, int max)
{
    int randi = dlb_rand() % ((max - min) + 1) + min;
    return randi;
}
// min and max are both inclusive bounds
float dlb_rand_float(float min, float max)
{
    float randf = (dlb_rand() / (float)UINT32_MAX) * (max - min) + min;
    return randf;
}
...
static void dlb_rand_test() {
    ...
    for (int i = 0; i < 10000; i++) {
        int one_thru_four = dlb_rand_int(1, 4);
