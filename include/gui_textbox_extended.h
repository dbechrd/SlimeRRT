...
RAYGUIDEF Vector2 GuiTextBoxGetSelection(void)
{
    if (guiTextBoxState.select == -1 || guiTextBoxState.select == guiTextBoxState.cursor)
        ...
        return RAYGUI_CLITERAL(Vector2){ (float)guiTextBoxState.select, (float)(guiTextBoxState.cursor - guiTextBoxState.select) };
...
RAYGUIDEF void GuiTextBoxPaste(char *text, int textSize)
{
    ...
    if ((text != NULL) && (clipText != NULL) && (guiTextBoxState.cursor != -1))
    {
        ...
        int clipLen = (int)strlen(clipText); // We want the length in bytes
...
RAYGUIDEF bool GuiTextBoxEx(Rectangle bounds, char *text, int textSize, bool editMode)
{
    ...
    {
        bounds.width = (float)(GuiGetStyle(DEFAULT, TEXT_SIZE) + 2*GuiGetStyle(TEXTBOX, TEXT_INNER_PADDING));
    }

    // Center the text vertically
    float verticalPadding = (bounds.height - 2*GuiGetStyle(TEXTBOX, BORDER_WIDTH) - GuiGetStyle(DEFAULT, TEXT_SIZE))/2;

    if (verticalPadding < 0)
    {
        // Make sure the height is sufficient
        bounds.height = (float)(2*GuiGetStyle(TEXTBOX, BORDER_WIDTH) + GuiGetStyle(DEFAULT, TEXT_SIZE));
        verticalPadding = 0;
    }
    ...
    if ((state != GUI_STATE_DISABLED) && !guiLocked)
    {
        ...
        if (editMode)
        {
            ...
            if (active)
            {
                ...
                cursorPos.x = (float)GuiTextBoxGetCursorCoordinates(&text[guiTextBoxState.index], length - guiTextBoxState.index, textRec, guiTextBoxState.cursor - guiTextBoxState.start);
    ...
    float borderWidth = (float)GuiGetStyle(TEXTBOX, BORDER_WIDTH);
    Rectangle innerRec = bounds;
    innerRec.x += borderWidth;
    innerRec.y += borderWidth;
    innerRec.width -= borderWidth * 2.0f;
    innerRec.height -= borderWidth * 2.0f;

    if (state == GUI_STATE_PRESSED)
    {
        DrawRectangleRec(innerRec, Fade(GetColor(GuiGetStyle(TEXTBOX, BASE_COLOR_FOCUSED)), guiAlpha));
    ...
    {
        DrawRectangleRec(innerRec, Fade(GetColor(GuiGetStyle(TEXTBOX, BASE_COLOR_DISABLED)), guiAlpha));
    }

    // Finally draw the text and selection
    DrawTextRecEx(guiFont, &text[textStartIndex], textRec, (float)GuiGetStyle(DEFAULT, TEXT_SIZE), (float)GuiGetStyle(DEFAULT, TEXT_SPACING), false, Fade(GetColor(GuiGetStyle(TEXTBOX, TEXT + (state*3))), guiAlpha), selStart, selLength, GetColor(GuiGetStyle(TEXTBOX, COLOR_SELECTED_FG)), GetColor(GuiGetStyle(TEXTBOX, COLOR_SELECTED_BG)));
...
static int GuiMeasureTextBox(const char *text, int length, Rectangle rec, int *pos, int mode)
{
    ...
    *pos = k;
    ...
    return ((int)rec.x + textOffsetX - 1);
}
